    ...
    <div class="dashbordHome">
        ...
        <div class="partyContent">
            ...
            <div class="sectionConteent">
                <div class="titleAjoute">Payment Client
            ...
            @endif
                ...
                <div class="sectionAjouteporduit">
                    <form action="{{ route('Pay_client_add') }}" method="post">
                        @csrf
                        @php
                                $dataPrixTotal=0;
                                $ItemClient=null;
                        ...
                        @endphp
                         <div class="unitearty">
                                <ul>
                                    ...
                                    <li>
                                        @foreach ($dataVente_deClient  as $key => $ItemClient)
                                            @php
                                                $ItemClient=$ItemClient->clients->nom_Complet;
                                            @endphp
                                        @endforeach
                                        <input type="text" name="PayClient" id="PayClient" value="{{$ItemClient}}"  placeholder="Nom Complet">
                                      </li>
                                </ul>
                            </div>
                            <div class="unitearty">

                            @foreach ($dataVente_deClient as $key => $itmPrix)
                              @php
                                  $dataPrixTotal += $itmPrix->Prix_Vente
                              @endphp
                            @endforeach
                                <ul>

        ...
        <div class="partyContent">
            ...
            <div class="sectionConteent">
                <div class="titleAjoute">
                   Client
                    <a class="btnAjouteaa" style="right: 0px" href="{{ route('Ajoute_Client') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                    ...
                    @else
                        <table id="tbld" style="padding: 10px">
                            ...
                            <tbody class="table">
                                    ...
                                    @foreach ($dataClient as $key=> $itmClient)
                                    <tr>
                                            <td>{{$itmClient->id}}</td>
                                            <td >{{  $itmClient->nom_Complet}}</td>
                                            <td >{{Str::limit($itmClient->Adresse, 10, '...')  }}</td>

                                            <td>{{Str::limit($itmClient->email, 10, '...')  }}</td>
                                            <td>{{Str::limit(  $itmClient->telephone, 6, '...')}}</td>
                                            <td>{{  $itmClient->Montant}}</td>

                                            <td>
                                                <form action="{{ route('Ajoute_Client_dl') }}" onsubmit="myfunsubmt(event)" method="post" id="formSumb">
                                                    @csrf
                                                    @method('delete')
                                                    <input type="hidden"  name="NameClient" value="{{ $itmClient->nom_Complet }}">
                                                    <input type="hidden"  id="idClient"  name="idClient" value="{{ $itmClient->id }}">

                ...
                <div class="titleAjoute">
                    ...
                    <div class="bntAjoute">
                        {{-- <select name="selectproduit" id="selectproduit" onchange="myfunselect(event)">
                            ...
                            <form action="{{ route('Devis_deleteAll.page') }}" onsubmit="myfunsubmt(event)" method="post" id="formSumb">
                                @csrf
                                @method('delete')
                                <input type="hidden"  name="NameClient" value="">
                                <input type="hidden"  id="idClient"  name="idClient" value="">
                                <button type="submit" class="btnSupprimeDEvis" >Supprime</button>
                            </form>
            ...
            @endif
                ...
                <div class="sectionAjoutegroupporduit">
                    <div class="sectionTable">
                        ...
                        <div class="notifactionRemove" id="notifactionRemove2">
                            <dotlottie-player
                                src="https://lottie.host/80c5f992-fb3c-4536-a947-1544a3e66f53/bkra6CPmBm.json"
                                background="transparent"
                                speed="0.4"
                            ...
                            <div class="message" id="message2">
                                Voulez-vous vraiment supprimer ce produit? <span id="NameUSer2" style="color: chocolate"></span>
                            </div>
                            <div class="bntgourpe">
                                <a id="btnAnnule" onclick="myfunsubmtAnuule2()">Annuler</a>
                                <a id="btnConfir" dataID='' onclick="myfunsubmtConfi2(event)">Oui</a>
                            </div>
                        </div>
                        ...
                        <table id="tbld">
                            ...
                            <tbody class="table">
                                ...
                                @if(count($dataDevis)==0)
                                        <tr>
                                            <td colspan="5">
                                            Oops Aucun produit dans cet  groupe de devis
                                            </td>
                                        </tr>

                                @elseif ($dataDevis)
                                            @foreach ($dataDevis as $key => $dataItm)

                                            <tr>
                                                {{-- <td>{{  $key+1 }}</td> --}}
                                                <td>{{  $dataItm->Description}}</td>
                                                <td>{{ $dataItm->Prix_unitaire_HT }}</td>
                                                <td>{{ $dataItm->Quantite }}</td>
                                                <td id="prix">{{  $dataItm->Unitaire_HT }}</td>
                                                <td>
                                                    <form action="{{ route('Devis_delete.page') }}" onsubmit="myfunsubmt2(event)" method="post" id="formSumb2">
                                                    @csrf
                                                    @method('delete')
                                                    <input type="hidden" name="id_devis" value="{{ $dataItm->id }}">
                                                    <input type="hidden"  value="{{ $dataItm->Description }}">
                                                    <input type="hidden"  id="idClient"  value="{{ $dataItm->id }}">
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="5">
                                        <form action="{{ route('Devis_Ajoute.page') }}" method="post">
                                            @csrf
                                            @method('get')
                    ...
                    <div class="sectionverif">
                        <div class="Totalproduit">Total produit : <span id="NumProduit">3</span></div>
                                ...
                                @if($dataDevis->ToArray())

                                    <form action="{{ route('Devis_ImprimPdf.page') }}" method="post">
                                        @csrf

                                            <button id="btnpdf" type="submit">Imprime Pdf </button>

                                    </form>
                                    <form action="{{ route('Devis_ImprimTickt.page') }}" method="post">
                                        @csrf

                                            <button  id="btnticket" type="submit">Ticket</button>

                                    </form>
                                    @else
                                        <a  class="btnform">Le groupe est vide</a>
                                @endif
...
<script>
    ...
    const notifactionRemove2=document.getElementById('notifactionRemove2')
    const message=document.getElementById('message')
    const message2=document.getElementById('message2')
    const btnConfir=document.getElementById('btnConfir')
    const formSumb=document.getElementById('formSumb')
    const formSumb2=document.getElementById('formSumb2')
    ...
    const  myfunsubmt = (event) =>{
        ...
        NameUSer.innerHTML=event.target[3].value;
        btnConfir.setAttribute('dataID',event.target[3].value)
        console.log(event.target[3].value);
        message.innerHTML=`Voulez-vous vraiment supprimer ce tout ? <span id="NameUSer" style="color: chocolate">${event.target[2].value}</span>`
    ...
    const  myfunsubmtConfi= (event) =>{
        ...
        if(event.target.hasAttribute('data-confirm')){
            ...
            message.innerHTML=`Tout les produit <span id='NameUSer' style="color: chocolate">${NameUSer.innerHTML} </span> va être supprimé maintenant. Veuillez confirmer, s'il vous plaît.`
    ...
    //party 2
    const  myfunsubmt2= (event) =>{
    event.preventDefault();
    NameUSer2.innerHTML=event.target[3].value;
    btnConfir.setAttribute('dataID',event.target[3].value)
   console.log(event.target[3].value);
    message2.innerHTML=`Voulez-vous vraiment supprimer ce Porduit ? <span id="NameUSer" style="color: chocolate">${event.target[3].value}</span>`
    btnConfir.innerHTML='Oui'
    btnConfir.style.background='#3965E3'
    btnConfir.removeAttribute('data-confirm')
    notifactionRemove2.style.display='flex';
    console.log(NameUSer);
  }
  const  myfunsubmtAnuule2 = () =>{
        btnConfir.removeAttribute('data-confirm')
        notifactionRemove2.style.display='none';
        }
  const  myfunsubmtConfi2= (event) =>{
...
if(event.target.hasAttribute('data-confirm')){
    ...
    // formSumb.action=;
    // formSumb.action="Devis_delete.page"
    // formSumb2.formAction="Devis_delete.page"

    console.log(formSumb2.action);
    formSumb2.submit();


}else{
    message2.innerHTML=`Produit <span id='NameUSer' style="color: chocolate">${NameUSer2.innerHTML} </span> va être supprimé maintenant. Veuillez confirmer, s'il vous plaît.`
    event.target.setAttribute('data-confirm','ConfirmNow');
    event.target.innerHTML='confirmer';
    event.target.style.background='#05d22ee8';
